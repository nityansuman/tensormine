...
class SqueezeFireModule(keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        """
        self.fire_conv = keras.layers.Conv2D(filters=int(self.fire_nodes[0]), kernel_size=(1, 1),
                                             strides=(1, 1), activation=None, padding="same")
        self.fire_batch = keras.layers.BatchNormalization()
        self.fire_relu = keras.layers.Activation("relu")
        self.fire_conv_1 = keras.layers.Conv2D(filters=int(self.fire_nodes[1]), kernel_size=(1, 1),
                                               strides=(1, 1),activation=None, padding="same")
        self.fire_batch_1 = keras.layers.BatchNormalization()
        self.fire_conv_2 = keras.layers.Conv2D(filters=int(self.fire_nodes[2]), kernel_size=(3, 3),
                                               strides=(1, 1), activation=None, padding="same")
    ...
    def call(self, x):
        ...
        if self.skip_connection:
            x = self.skip_connection_output()([x, temp_in])
